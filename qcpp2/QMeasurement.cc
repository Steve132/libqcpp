...
void MEASUREMENT::apply(QMachine& qm)
{
	...
	this->kernel.setArg(2,mask);
	this->kernel.setArg(3,rbegin);
	this->kernel.setArg(4,1.0f/sqrt(norm_factor));
	
	QGate::apply(qm);
		
	msbshift(qm,-bits_used);		//unshift to msb
}

std::string MEASUREMENT::gatesrc="";
std::string MEASUREMENT::tsrc=R"CLSRC(

__kernel void gateapply(__global float2* outbits,const __global float2* inbits,const ulong mask,const ulong resultmask,const float Px)
{
